import React, { createElement, createRef, Component, cloneElement } from 'react';
...
function SortableRecursive(props) {
    ...
    return (React.createElement(ReactSortable, __assign({ uncontrolled: true }, options, handlers, { list: list }), list.map(function (item) {
        // todo:
        // allow a ref?
        // parse details like `depth` and `index` and `path` to the `children` function
        var Nested = function () { return (React.createElement(SortableRecursive, __assign({}, props, { children: children, depth: depth + 1, 
            //@ts-ignore
            list: item.children || [] }))); };
...
function ReactSortableNested(props) {
    ...
    return React.createElement(SortableRecursive, __assign({ setRootState: setList, list: list, depth: 0 }, otherProps));
}
